...
- 涉及题材定位、世界规则、高阶内容升格、长期系统演进或修真 / 武侠取舍时，先看 `docs/修真题材长期总纲.md`。
- 涉及系统分期、功能前后顺序、当前阶段主线与高阶内容是否该提前放开时，再看 `docs/修真系统分期总表.md`。
- 若其他设计文档在题材与世界规则上出现局部措辞冲突，以 `docs/修真题材长期总纲.md` 为准；本文继续负责工程、架构和实现边界。
...
- `src/config/` 下的主配置文件必须按“主类 / 子系统 / 主题分段”组织，优先使用分组数组或明确的区块注释，再在文件底部统一汇总导出。
- 禁止在超长配置文件尾部无序追加条目；新增内容必须归入对应分组。
- Map、索引、辅助 getter 一律放在配置分组之后，避免数据和派生结构交错。
- `items.ts`、`techniques.ts`、`economy.ts` 属于高频维护文件，新增内容时必须优先保持可读性，而不是只追求“能跑”。

### 1.4 物品文档回写

- 任何物品、装备、秘籍、已学功法、悟道配方相关改动，必须同步重写 `docs/item.md`。
- `docs/item.md` 至少保持四类内容同步：分类总表、品阶表、分类型明细表、功法定义与悟道配方。
- 若新增的是发现专属秘籍或特殊获取链路，必须在文档中明确标注，不得只改代码不回写说明。
