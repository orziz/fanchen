(() => {
  ...
  function adjustResource(key, amount, maxKey) {
    const game = app.getGame();
    const current = game.player[key];
    if (typeof current !== "number") return;
    const max = maxKey && typeof game.player[maxKey] === "number" ? game.player[maxKey] : Infinity;
    game.player[key] = clamp(current + amount, 0, max);
  }

  function getAssetCollection(kind) {
    return app.getGame().player.assets[`${kind}s`] as ShanHaiAssetState[];
  }
