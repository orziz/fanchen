(() => {
  ...
  function getClosest(target, selector) {
    if (target instanceof HTMLElement) return target.closest(selector) as HTMLElement | null;
    if (target && target.parentElement instanceof HTMLElement) return target.parentElement.closest(selector) as HTMLElement | null;
    return null;
  }
