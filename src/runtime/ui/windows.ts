(() => {
  ...
  function getWindowBody(windowId) {
    return getWindowElement(windowId)?.querySelector<HTMLElement>(".window-body") || null;
  }

  function getWindowToggleButton(windowId) {
    return document.querySelector<HTMLElement>(`[data-open-window="${windowId}"]`);
  }

  function snapshotWindowLayout() {
    return {
      windowZCounter: state.windowZCounter || 3,
      windows: WINDOW_IDS.reduce<Record<string, ShanHaiWindowState>>((result, windowId) => {
  ...
  function ensureWindowState(windowId) {
    ...
    if (windowState.dockSide === undefined) {
  ...
  function renderFloatingWindows() {
    ...
    document.querySelectorAll<HTMLElement>("[data-window-action=\"minimize\"]").forEach((button) => {
    ...
    document.querySelectorAll<HTMLElement>("[data-window-action=\"dock\"]").forEach((button) => {
      const windowState = ensureWindowState(button.dataset.windowId);
