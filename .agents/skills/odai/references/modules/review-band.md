...
8. 满足 `references/dao/parallel-consensus-playbook.md` 的完整能力条件时，才允许并行开启完整乐队审查；若完整条件不成立、但部分能力条件成立，则允许退而进入“子 agent 辅助复核模式”，只拉 1 至 2 个子 agent 做题面校对、反例挑战、补证据或冻结结论后的辅助复查。内部收束、冻结结论后的独立复查与分歧升级统一按该手册执行；辅助复核不得包装成完整乐队审查或多模型参与；两档条件都不成立时，一律按原单流程完成。
